...
`KeyExchange` agree a shared key as `g ^ q2 ^ q1` and `g ^ q1 ^ q2`, where
`q1` and `q2` are random exponents generated by the two parties. And
one-time pad encryption is realized using the group operation, where
`k ^^ (g ^ q2 ^ q1) ^^ kinv (g ^ q1 ^ q2)` simplifying to `k`.
