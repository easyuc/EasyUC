...
type messageDef =
  {direction : msgInOut; id : id; content : nameType list;
   portLabel: id option}
...
type msgMatch =
  {portVar : id option; path : msgPath; tupleMatch : matchItem list option}
...
type msgInstance =
  {path : msgPath; tupleInstance : expressionL list; portVar : id option}
...
type funDef =
  {id : id; params : funParam list; idDirIO : id; idAdvIO : id option;
   body : subItem list; stateBody : stateDef list}

type simDef =
  {id : id; uses : id; sims : id; simsParamIds: id list;
   body : stateDef list }
