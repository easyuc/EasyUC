...
let check_circ_refs (get_refs:('o located) IdMap.t-> string -> IdSet.t) (os:('o located) IdMap.t) : unit =
...
let check_params (n_tl:name_type list) : typ_tyd IdMap.t =
                ...
                IdMap.map (fun (nt:name_type) -> mk_loc (loc nt.id) ((check_type nt.ty), (index_of_ex nt n_tl))) nt_map
...
(*IO checks*)
let check_exists_io (ermsgpref: string) (e_io:string->bool) (io_i:io_item) : io_item_tyd =
...
Basic (IdMap.map (fun (md:message_def) ->mk_loc (loc md.id) {direction = md.direction; content = (check_params md.content); port_label = md.port_label}) msg_map)
...
let check_adi_os (errmsgpref:string) (ad_io_map: io IdMap.t) =
        ...
        let check_adio_def (io:io) : io_tyd = 
                ...
                | Composite iob -> mk_loc (loc io.id) (check_comp_io_body errmsgpref e_io iob)
        in
        let ad_i_os = IdMap.map check_adio_def ad_io_map in
        check_composites_ref_basics ad_i_os;
        ad_i_os

let check_dir_i_os (dir_io_map: io IdMap.t) = check_adi_os "direct_io" dir_io_map

let check_adv_i_os (adv_io_map: io IdMap.t) = check_adi_os "adversarial_io" adv_io_map
...
let check_sub_fun_decl (e_f_id:string->bool) (e_param:string->bool) (e_sf_id:string->bool)(sf:sub_fun_decl) : sub_fun_decl_tyd =
...
let check_state_decl (init_id:id) (s:state) : state_tyd =
        ...
        let vars = IdMap.map (fun tip -> mk_loc (loc tip) (fst (unloc tip))) (check_params s.code.vars) in
...
type r_fb_io_paths = {direct:b_io_path list; adversarial:b_io_path list; internal:b_io_path list }

let getb_io_paths (root:string) (ioid:string) (ios:io_tyd IdMap.t) : b_io_path list =
        ...
        match (unloc io) with
        | Basic b -> [([root],b)]
        | Composite cio -> IdMap.fold (fun id it l-> ([root;id],getb_body (unloc (unloc it)))::l) cio []
...
let get_fun_io_paths (id_dir_io:string) (id_adv_io:string option) (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t) : string list list =
        let dir = get_io_paths id_dir_io dir_i_os in
        let adv = match id_adv_io with
                  | Some id -> get_io_paths id adv_i_os
                  | None -> []
        in dir@adv

let check_i_opath (id_dir_io:string) (id_adv_io:string option) (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t) (iop: id list) : string list located =
...
let check_i_os_cover (id_dir_io:string) (id_adv_io:string option) (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t) (served_ps: string list located list): unit =
...
let check_party_decl (id_dir_io:string) (id_adv_io:string option) (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t)(p:party_def) : party_def_tyd =
        ...
        check_served_paths serves id_dir_io p.id;
        let code = check_states p.id p.code in
        mk_loc (loc p.id) {serves=serves; code=code} 

let check_parties_serve_direct_sum (parties:party_def_tyd IdMap.t) (id_dir_io:string) (id_adv_io:string option) (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t): unit =
        let served_ps = IdMap.fold (fun _ p l -> l@(unloc p).serves) parties [] in
        check_i_os_unique served_ps;        
...
let check_fun_decl (e_f_id:string->bool) (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t) (r_fun:fun_def) : fun_tyd =
        ...
        check_exists_dir_io dir_i_os r_fun.id_dir_io;
        let id_dir_io = unloc r_fun.id_dir_io in 
        let id_adv_io = match r_fun.id_adv_io with
                        | None -> None
                        | Some id -> 
                                (
                let uid = unloc id in
        ...
        let sub_funs = IdMap.map (check_sub_fun_decl e_f_id e_param e_sf_id) sf_map in
        let parties,states =
                check_is_composite dir_i_os r_fun.id_dir_io;
                if r_fun.state_body=[] then(                      
                        (match r_fun.id_adv_io with Some id -> check_is_composite adv_i_os id|_->());
                        let p_map = filter_map (fun sub_i -> match sub_i with PartyDef p ->Some p | _->None) sub_items in
                        let ps = IdMap.map (check_party_decl id_dir_io id_adv_io dir_i_os adv_i_os) p_map in
                        check_parties_serve_direct_sum ps id_dir_io id_adv_io dir_i_os adv_i_os;
                        (ps,IdMap.empty))
                else
                        ...
                        (IdMap.empty,ss)
        in
        mk_loc (loc r_fun.id) {params=params; id_dir_io=id_dir_io; id_adv_io=id_adv_io; sub_funs=sub_funs; parties=parties; states=states}

let get_dir_io_id_impl_by_fun (fid:string) (funs:fun_tyd IdMap.t) : string =
                        let r_f=IdMap.find fid funs in (unloc r_f).id_dir_io
                        

let get_param_dir_io_ids (r_funs:fun_tyd IdMap.t) (rfid:string) :string list = 
...
let check_sub_fun_params (funs:fun_tyd IdMap.t) : unit =
        let get_dir_io_id rfid id =                
                ...
                let s = IdMap.find_opt id f.sub_funs in
                        ...
                        | _ -> raise (Failure "Impossible - we already checked that sub_fun params exist and are unique in check_sub_fun_decl")
        in
        let get_dir_io_ids rfid ids = List.map (fun id -> get_dir_io_id rfid id) ids in
        ...
        ignore (IdMap.for_all 
                (fun idrf rf -> IdMap.for_all 
                        (fun _ sf -> check_sf_ps idrf sf) (unloc rf).sub_funs
                ) funs)

type state_vars = {flags:string list; internal_ports:QidSet.t; consts:typ IdMap.t; vars:typ IdMap.t; initialized_vs:IdSet.t}

let init_state_vars (s:state_body) (ports: QidSet.t) (flags:string list): state_vars =
        let consts = IdMap.map (fun p -> fst (unloc p)) s.params in
        let vars = IdMap.map (fun v -> unloc v) s.vars in
        {flags=flags; internal_ports=ports; consts=consts; vars=vars; initialized_vs=IdSet.empty}
...
let filterb_io_ps (dir:msg_in_out) (biops:b_io_path list) : b_io_path list =
...
IdMap.fold (fun id _ l -> { io_path=dummylocl (fst bp); msg_type=MsgType (dummyloc id) }::l ) (snd bp) []
...
let check_msg_path (bps:r_fb_io_paths) (mp:msg_path) : msg_path =
        ...
        let filter_by_msg_type (mt:id) (mpl:msg_path list) : msg_path list =
                List.filter (fun p-> match p.msg_type with MsgType mt' -> (unloc mt')=(unloc mt) | _->false) mpl
        in
        let filter_by_port_name_msg_type (pt:id) (mt:id) (mpl:msg_path list) : msg_path list =
filter_by_msg_type mt (List.filter (fun p-> (unloc (List.hd(List.rev p.io_path)))=(unloc pt)) mpl)
        ...
        | MsgType mt ->
                (
                if (List.exists (fun p -> (string_of_msg_path p)=(string_of_msg_path mp)) allps) then mp
                else
                match (List.length mp.io_path) with
                | 0 ->  let filter = filter_by_msg_type mt in
                        let mtch = filter allps in
                        let imtch = filter ips in
                        filtered mtch imtch
                | 1 ->  let filter = filter_by_port_name_msg_type (List.hd mp.io_path) mt in
                        let mtch = filter allps in
                        let imtch = filter ips in
                        filtered mtch imtch
                | _ ->  unexpected()
                )
        | OtherMsg _ -> if (List.exists (fun p-> qid1_starts_with_qid2 p.io_path mp.io_path) allps) then mp else unexpected()

let remove_covered_paths (mps:msg_path list) (mp:msg_path) : msg_path list =
        let covered mp1 mp2 =
                match mp2.msg_type with
                | MsgType _ -> ((string_of_msg_path mp1)=(string_of_msg_path mp2))
                | OtherMsg _ -> qid1_starts_with_qid2 mp1.io_path mp2.io_path
        in
...
let msg_paths_of_r_fb_io_paths_w_othermsg (bps:r_fb_io_paths) : msg_path list =
        ...
        let omps = List.map (fun bp -> { io_path=dummylocl (fst bp); msg_type=OtherMsg _dummy }) (flatten_r_fb_io_paths bps) in
        mps@omps

let check_mm_ds_non_empty (bps:r_fb_io_paths) (mpl:msg_path list) : msg_path list =
        let mps = msg_paths_of_r_fb_io_paths_w_othermsg bps in
        List.fold_left (fun mps mp -> remove_covered_paths mps mp) mps mpl


let check_msg_match_deltas (rfbps:r_fb_io_paths) (mml:msg_match list) : unit =
...
let check_exists_and_has_compatible_type (vid:id) (typ:typ) (sv:state_vars) : unit =
...
let check_type_add_binding (vid:id) (typ:typ) (sv:state_vars) : state_vars =
        check_exists_and_has_compatible_type vid typ sv;
        check_add_binding vid sv

let check_add_const (cid:id) (ct:typ) (valt:typ) (sv:state_vars) : state_vars =
        check_types_compatible cid ct valt;
...
let check_item_type_add_binding (sv:state_vars) (mi:match_item) (typ:typ) : state_vars =
        match mi with
        | Wildcard _ -> sv
...
let get_loc_match_item_list (tm:match_item list) : EcLocation.t = mergeall (List.map (fun mi ->get_loc_match_item mi) tm)

let check_msg_content_bindings (ps:b_io_path list) (mp:string list*string) (tm:match_item list) (sv:state_vars) : state_vars =
...
let check_tuple_match (bps:b_io_path list) (mm:msg_match) (sv:state_vars) : state_vars =
...
let check_expr_var (sv: state_vars) (id:id) : typ =
        ...
        check_initialized sv id;
        r

let check_is_internal_port (sv: state_vars) (qid:qid) : bool = QidSet.mem (unlocs qid) sv.internal_ports
...
let check_val_assign (sv:state_vars) (vid:id) (ex:expression_l) : state_vars =
...
let check_send_adversarial (msg:msg_instance) (mc:typ_tyd IdMap.t) (sv:state_vars) : unit =
        let l=msg_loc msg.path in
        (
        match msg.port_var with
...
let check_send_internal (msg:msg_instance) (mc:typ_tyd IdMap.t) (sv:state_vars) : unit =
        let l=msg_loc msg.path in
        (
        match msg.port_var with
...
let get_msg_def_for_msg_path (mp:msg_path) (bs:b_io_path list) : message_def_body =
        let iop = unlocs mp.io_path in
        let bio = List.find (fun bp -> (fst bp)=iop) bs in
        let mt  = match mp.msg_type with MsgType id -> unloc id | OtherMsg _ -> raise (Failure "OtherMsg doesn't have definition in interface") in
        let mdb = IdMap.find mt (snd bio) in
        unloc mdb

let check_send_msg_path (msg:msg_instance) (bps:r_fb_io_paths) (sv:state_vars) : msg_instance =
...
let check_send (msg:msg_instance) (bps:r_fb_io_paths) (sv:state_vars) : msg_instance =
        ...
        (
        match msg'.path with
        | ( _ as p) when is_msg_path_inb_io_paths p bps.direct -> check_send_direct msg' mdbc sv
        | ( _ as p) when is_msg_path_inb_io_paths p bps.adversarial -> check_send_adversarial msg' mdbc sv
        | ( _ as p) when is_msg_path_inb_io_paths p bps.internal -> check_send_internal msg' mdbc sv
        | _ -> raise (Failure "impossible - the path is always in one of direct|adversarial|internal ")
        );
        msg'

let check_send_and_transition (bps:r_fb_io_paths) (ss:state_sig IdMap.t) (sat:send_and_transition) (sv:state_vars)  : instruction =
        let msg'= check_send sat.msg bps sv in
        check_transition sat.state ss sv;
        SendAndTransition {msg=msg'; state=sat.state}

let merge_state_vars (sv1:state_vars) (sv2:state_vars) : state_vars =
        {flags=sv1.flags; internal_ports=sv1.internal_ports; consts=sv1.consts; vars=sv1.vars; initialized_vs=IdSet.inter sv1.initialized_vs sv2.initialized_vs}
...
check_branches (bps:r_fb_io_paths) (ss:state_sig IdMap.t) (sv:state_vars) (tins:instruction_l list) (eins:instruction_l list option) : instruction_l list * instruction_l list option * state_vars =
        let (tins_c,tsv) = check_instructions bps ss sv tins in
        let (eins_c,esv) = match eins with                         
                          | Some is -> let (is',esv) = check_instructions bps ss sv is
                                        in (Some is',esv)
                          | None -> (None,sv)
        in
...
and
check_instruction (bps:r_fb_io_paths) (ss:state_sig IdMap.t) (insv:instruction_l list*state_vars)  (i:instruction_l): instruction_l list*state_vars =
        let ins=fst insv in
        let sv =snd insv in
        match (unloc i) with
        | Assign (vid,ex) -> (ins@[i]),(check_val_assign sv vid ex)
        | Sample (vid,ex) -> (ins@[i]),(check_sampl_assign sv vid ex)
        | ITE (ex,tins,eins) -> let (ite_c,sv') = check_ite bps ss sv ex tins eins in
                                (ins@[mk_loc (loc i) ite_c],sv')
        | Decode (ex,ty,m_is,okins,erins) -> let (match_c,sv') = check_decode bps ss sv ex ty m_is okins erins in
                                (ins@[mk_loc (loc i) match_c],sv')
        | SendAndTransition sat -> (ins@[mk_loc (loc i) ( check_send_and_transition bps ss sat sv)]),sv
        | Fail -> (ins@[i]),sv

and

check_instructions (bps:r_fb_io_paths) (ss:state_sig IdMap.t) (sv:state_vars) (is:instruction_l list) : (instruction_l list * state_vars) =
...
let check_msg_code (bps:r_fb_io_paths) (ss:state_sig IdMap.t) (sv:state_vars) (is:instruction_l list) : instruction_l list =
        ...
        check_ends_are_sa_tor_f is_tyd;
        is_tyd

let check_message_path (bps:r_fb_io_paths) (mmc:msg_match_code) : msg_match_code =
        let path' = check_msg_path (get_incoming_msg_paths bps) mmc.pattern_match.path in
        {pattern_match={port_var=mmc.pattern_match.port_var; path=path'; tuple_match=mmc.pattern_match.tuple_match}; code=mmc.code}

let check_m_mcode (bps:r_fb_io_paths) (ss:state_sig IdMap.t) (sv:state_vars) (mmc:msg_match_code) : msg_match_code =
...
let check_state_code (bps:r_fb_io_paths) (ss:state_sig IdMap.t) (sv:state_vars) (mmcodes:msg_match_code list) : msg_match_code list =
        let mmcodes' = List.map (fun mmc -> check_m_mcode bps ss sv mmc) mmcodes in
        check_msg_match_deltas bps (List.map (fun mmc->mmc.pattern_match) mmcodes');
        mmcodes'
...
let get_internal_ports (r_f:fun_body) : QidSet.t =
        QidSet.union (get_keys r_f.parties) (QidSet.union (get_keys r_f.params) (get_keys r_f.sub_funs))

let filterb_io_paths (bps:b_io_path list) (pfxs:string list located list) : b_io_path list =
        List.filter (fun bp -> List.exists (fun pfx -> (unloc pfx) = (fst bp) ) pfxs) bps

let get_fb_io_paths (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t) (f:fun_tyd) : r_fb_io_paths =
        let uf = unloc f in
...
let get_r_fb_io_paths (dir_i_os:io_tyd IdMap.t) (adv_i_os:io_tyd IdMap.t) (funs:fun_tyd IdMap.t) (r_f:fun_tyd) (p:party_def_tyd) : r_fb_io_paths =
        ...
        let internal_sfm = IdMap.mapi (fun sfid (sf:sub_fun_decl_tyd) -> 
let did = get_dir_io_id_impl_by_fun ((unloc sf).fun_id) funs in
getb_io_paths sfid did dir_i_os ) ur_f.sub_funs in
        let internal_pm = IdMap.mapi (fun pid p -> 
                let did = unloc (unloc (fst p)) in
getb_io_paths pid did dir_i_os ) ur_f.params in
        let internal_m = IdMap.union (fun _ _ _ ->raise (Failure "Impossible, we already checked params and subfuns have different ids")) internal_sfm internal_pm in
        let internal = IdMap.fold (fun _ bps l -> l@bps) internal_m [] in
        {direct=direct; adversarial=adversarial; internal=internal}

let check_state (ur_f:fun_body) (states:state_tyd IdMap.t) (bps:r_fb_io_paths) (s:state_tyd) : state_tyd =
        let us = unloc s in
        ...
        mk_loc (loc s) {is_initial=us.is_initial; params=us.params; vars=us.vars; mmcodes=mmcodes'}
...
let check_party_code dir_i_os adv_i_os funs =
        IdMap.map 
                ...
                let parties'= IdMap.map 
                        ...
                        let states'= IdMap.map (check_state ur_f states bps) states
                        in
                ...
                let states'= IdMap.map (check_state ur_f states bps) states
                in
                mk_loc (loc r_f) {params=ur_f.params; id_dir_io=ur_f.id_dir_io; id_adv_io=ur_f.id_adv_io; sub_funs=ur_f.sub_funs; parties=parties'; states=states'}
                ) funs

let get_sf_refs_to_f_in_rf (funs:fun_tyd IdMap.t) (fid:string) : IdSet.t =
        let rfb = IdMap.find fid funs in
        let sfrf = IdMap.filter (fun _ r -> exists_id funs (unloc r).fun_id) (unloc rfb).sub_funs in
        IdMap.fold (fun _ r set -> IdSet.add (unloc r).fun_id set) sfrf IdSet.empty

let check_circ_refs_in_r_funs (rfs:fun_tyd IdMap.t) = check_circ_refs get_sf_refs_to_f_in_rf rfs

let check_funs (fun_map: fun_def IdMap.t) (dir_i_os: io_tyd IdMap.t) (adv_i_os: io_tyd IdMap.t) : fun_tyd IdMap.t =
...
let check_msg_code_sim (fbps: r_fb_io_paths) (ss: state_sig IdMap.t) (mmc:msg_match_code) (sv: state_vars) : msg_match_code =
        let code'= check_msg_code fbps ss sv mmc.code in
        {pattern_match=mmc.pattern_match; code=code'}

let check_message_path_sim (bps: b_io_path list) (isini:bool) (mmc:msg_match_code) : unit =
...
let check_match_bindings_sim (bps: b_io_path list) (sv: state_vars) (mmc:msg_match_code) : state_vars =
        let mm = mmc.pattern_match in
        check_tuple_match bps mm sv

let check_msg_match_deltas_sim (rfbps:r_fb_io_paths) (mmcodes:msg_match_code list) : unit =
        let mps = get_incoming_msg_paths rfbps in
        ignore (check_mm_ds_non_empty mps (List.map (fun mmc->{io_path=mmc.pattern_match.path.io_path; msg_type=mmc.pattern_match.path.msg_type}) mmcodes))

let check_sim_state_code (bps: b_io_path list) (ss: state_sig IdMap.t) (sv: state_vars) (isini:bool) (mmcodes:msg_match_code list) : msg_match_code list =
        List.iter (check_message_path_sim bps isini) mmcodes;
        ...
        check_msg_match_deltas_sim fbps ret;
        ret
...
let get_sim_components (funs: fun_tyd IdMap.t) (r_f:string) (ps:string list) : fun_body QidMap.t =
        let rec get_sc (funs: fun_tyd IdMap.t) (fid:string) (pfx:SL.t) : fun_body QidMap.t =
                ...
                then    
                        ...
                        ::
                        (IdMap.fold (fun sfid sfd l-> (get_sc funs (unloc sfd).fun_id (pfx@[sfid]))::l ) urf.sub_funs [])
                        )
        ...
        (get_sc funs r_f [r_f])
        ::
        (List.map2 (get_sc funs) ps qpids)
        )
                
let get_component_io_paths (adv_i_os: io_tyd IdMap.t) (f:fun_body) : b_io_path list =
...
let invert_mdf (mdf:message_def_body) : message_def_body = 
        {direction=(invert_dir mdf.direction); content=mdf.content; port_label=mdf.port_label}
...
let get_simb_io_paths (adv_i_os: io_tyd IdMap.t) (uses:string) (cs:fun_body QidMap.t) : b_io_path list =
        let sbps = QidMap.map (get_component_io_paths adv_i_os) cs in        
        let bps = QidMap.add [] (List.map (fun bp ->invert_msg_dirs bp) (getb_io_paths uses uses adv_i_os)) sbps in
...
let check_sim_code (_: io_tyd IdMap.t) (adv_i_os: io_tyd IdMap.t) (funs: fun_tyd IdMap.t) (sim:sim_def_tyd) : sim_def_tyd = 
        ...
        let states'= IdMap.map 
                (fun s ->
                let us = unloc s in
                let sv = init_state_vars us (get_sim_internal_ports cs) ["simulator"] in
                let mmcodes'= check_sim_state_code bps ss sv us.is_initial us.mmcodes in
                mk_loc (loc s) {is_initial=us.is_initial; params=us.params; vars=us.vars; mmcodes=mmcodes'}
                ) states
                in
        mk_loc (loc sim) {uses=usim.uses; sims=usim.sims; sims_param_ids=usim.sims_param_ids; body=states'}
...
let check_sim_decl (_: io_tyd IdMap.t) (i2s_i_os: io_tyd IdMap.t) (funs: fun_tyd IdMap.t) (sd: sim_def) : sim_def_tyd =
        check_exists_i2_sio i2s_i_os sd.uses;
        let uses = unloc sd.uses in
        check_is_real_f funs sd.sims;
        let sims = unloc sd.sims in
        List.iter (check_exists_f funs) sd.sims_param_ids;
        check_sim_fun_params funs i2s_i_os sd.sims sd.sims_param_ids;
        let sims_param_ids = unlocs sd.sims_param_ids in
        let body = check_states sd.id sd.body in
        mk_loc (loc sd.id) {uses=uses; sims=sims; sims_param_ids=sims_param_ids; body=body}

let check_simulators (sim_map: sim_def IdMap.t) (dir_i_os: io_tyd IdMap.t) (adv_i_os: io_tyd IdMap.t) (funs: fun_tyd IdMap.t) : sim_def_tyd IdMap.t =
...
let check_defs def_l =
                ...
                let dir_io_map = filter_map (fun def -> match def with IODef DirectIO io ->Some io | _->None) def_map in
                let adv_io_map = filter_map (fun def -> match def with IODef AdversarialIO io ->Some io | _->None) def_map in
                let fun_map = filter_map (fun def -> match def with FunDef f ->Some f | _->None) def_map in
                let sim_map = filter_map (fun def -> match def with SimDef sd ->Some sd | _->None) def_map in
                ...
                { direct_i_os = dir_i_os;
                  adversarial_i_os = adv_i_os;
...
let typecheck spec =
  UcEcInterface.init();
  load_ec_reqs spec.externals.ec_requirements;
