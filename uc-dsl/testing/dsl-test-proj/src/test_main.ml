...
let call_dir_test () =
  ...
  in
  if !debug then
    let x = List.nth !dirs_list 1 in
    if Sys.file_exists x then pre_debug x
    else if Sys.file_exists ("./" ^ x) then pre_debug ("./" ^ x)
    else (
      print_endline (x ^ " not found");
      exit 1)
  else
    let b = verify_dir (List.nth !dirs_list 1) in
    let _ = create_log () in
    pre_run b
...
let check_verbosity str =
  if (not (!verbose || !quiet || !debug)) && List.length !dirs_list = 0 then (
    insert_dirs str;
    0)
    ...
    print_endline (List.nth !dirs_list 0 ^ " is not allowed before -" ^ str);
    1)
  ...
  let rec speclist =
    [
      ( "-verbose",
        Arg.Unit
          (fun () ->
            if check_verbosity "verbose" = 0 then verbose := true
            else (
      ...
      ( "-quiet",
        Arg.Unit
          (fun () ->
            if check_verbosity "quiet" = 0 then quiet := true
            else (
              Arg.usage speclist usage_msg;
              exit 1)),
