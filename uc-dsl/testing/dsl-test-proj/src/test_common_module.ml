...
let print_expr (e : expr) =
  ...
  | Exec e -> print_string ("__exec__\n" ^ e ^ "\n__end of exec__\n")
  | Args o ->
      print_endline "__args__";
      List.iter print_endline o;
      print_endline "__end of ARGS__"
  | Outcome (o1, o2) ->
      let _ = print_endline "__outcome__" in
      let _ =
        if o1 = Success then print_endline "success"
      ...
      else print_endline "Warning: description missing"
...
let check_fields lst =
  let rec check arg exec desc out = function
    | [] -> (arg, exec, desc, out)
    | e :: l -> (
        match e with
        | Args _ -> check (arg + 1) exec desc out l
        | Exec _ -> check arg (exec + 1) desc out l
        | Desc _ -> check arg exec (desc + 1) out l
        | Outcome _ -> check arg exec desc (out + 1) l)
  in
  let arg_1, exec_1, desc_1, out_1 = check 0 0 0 0 lst in
  let s1 =
    if arg_1 <> 1 then
      if arg_1 = 0 then "Error: Missing Args" else "Error: Multiple Args"
    else ""
  in
  let s2 =
    if desc_1 <> 1 then
...
let get_desc lst =
  let rec desc lst_d str =
    match lst_d with
    | [] -> str
    | e :: l -> ( match e with Desc d -> desc l str ^ d | _ -> desc l str)
  in
  desc lst ""
...
let parse (file_name : string) =
  ...
  try
    let ctr = Test_parser.prog Test_lexer.my_lexer lexbuf in
    close_in ch;
    ctr
  with e ->
    close_in ch;
    ...
    let dirs, files =
      List.fold_left
        (fun (dirs, files) f ->
          ...
          | S_REG ->
              if String.sub f 0 1 = "." then (dirs, files)
                (* f is Hidden file *)
              else (dirs, f :: files)
      ...
      let rec walk_folders f_list t_list e_string =
        ...
        | e :: l ->
            let t_list_new, e_string_new =
              walk_directory_tree e t_list er_string
            in
...
let norm_stat stat = match stat with Unix.WEXITED n -> Some n | _ -> None
