...
functionality SMCIdeal implements SMCDir SMC2Simb {
  initial state WaitReq {
    match message with 
    | pt1@SMCDir.Pt1.smc_req(pt2, t) => {
        if (envport pt2) {
          send SMC2Simb.sim_req(pt1, pt2) and transition WaitSim(pt1, pt2, t).
        }
        else { fail. }
      }
    end
...
simulator SMCSim uses SMC2Simb simulates SMCReal(KeyExchange.KEIdeal) {
  initial state WaitReq {
    match message with 
    | SMC2Simb.sim_req(pt1, pt2) => {
  ...
  state WaitAdv3(pt1 : port, pt2 : port, q : exp) {
    ...
    | SMCReal.Fwd.FwAdv.fw_ok => {
        send SMC2Simb.sim_rsp and transition Final.
      }
    | *                       => { fail. }
    end
  }
