...
(* get1char reads a character without pressing enter, Sourse: Jeffrey Scofield, however this doesn't work in Windows; since tcgetattr is not implemented yet in windows *)
...
let create_log () =
  try
    ...
    let _ = if Sys.file_exists(folder^"/"^file_name) then
            (let _ =  print_endline
             "log file/fail log  already exists, press O/o to overwrite any other key to exit" in
